...
export default function ComparePage() {
  ...
  if (page.prompt.versions.length < 2) {
    return (
      <div className="page-wrap narrow-page">
        <EmptyState eyebrow="缺少挑战版本" title="至少需要两个版本" detail="返回工作台修改模板并保存为 V2，之后即可使用相同用例并行对比。" actionLabel="返回 Prompt" onAction={() => navigate(`/prompt/${id}`)} />
      </div>
    )
  }
  ...
  const winnerLabel = verdict?.winner === 'incomparable' ? '质量证据不足' : verdict?.winner === 'tie' ? '两版接近' : verdict?.winner === 'left' ? `V${resultSnapshot?.leftVersion?.versionNumber ?? leftVersion.versionNumber} 综合分较高` : `V${resultSnapshot?.rightVersion?.versionNumber ?? rightVersion.versionNumber} 综合分较高`
    ...
    <div className="page-wrap compare-page">
      ...
      <section className="panel compare-config">
        <header className="section-heading">
          <div><h2>测试设置</h2><p>{selectedSuite ? `套件「${selectedSuite.name}」` : '临时用例'} · {variables.length} 个变量 · {testCases.length} 个用例</p></div>
        </header>
        ...
        <button className="button button-primary run-button" disabled={running || !modelName.trim() || Number(leftId) === Number(rightId)} onClick={runComparison} type="button">
      ...
      {runs ? (
        <section className="comparison-results">
          <p className="muted-copy">本次运行：{resultSnapshot.provider} / {resultSnapshot.modelName} · {resultSnapshot.testCaseCount} 个用例</p>
          <header className="comparison-verdict">
            {gate ? <ReleaseGate verdict={gate} snapshot={resultSnapshot} /> : null}
            <div><h2>{winnerLabel}</h2><p>{verdict.winner === "incomparable" ? "至少一侧运行没有可用质量评分，无法计算综合分或选出赢家。" : "综合分 = 质量 + 断言通过率 − 延迟、成本与隐私风险；分差小于 1 判为接近。"}</p></div>
          ...
          <div className="comparison-table">
            ...
            <MetricRow label="断言通过率" left={runs.left.metrics.totalAssertions ? asPercent(runs.left.metrics.assertionPassRate) : '—'} right={runs.right.metrics.totalAssertions ? asPercent(runs.right.metrics.assertionPassRate) : '—'} delta={runs.left.metrics.totalAssertions && runs.right.metrics.totalAssertions ? signed(verdict.deltas.assertionPassRate * 100, 0, '%') : '—'} />
...
function VersionCard({ side, version, versions, value, onChange, disabled }) {
  return (
    <article className="version-card">
      <header><label htmlFor={`version-${side}`}>{side === 'A' ? '基线版本 A' : '候选版本 B'}</label><select id={`version-${side}`} disabled={disabled} value={value} onChange={(event) => onChange(event.target.value)}>{versions.map((item) => <option key={item.id} value={item.id}>V{item.versionNumber}</option>)}</select></header>
...
function ResultSide({ version, result }) {
  if (!result) return <section><p>该用例没有返回结果。</p></section>
  return (
    <section>
